...
export class StudentService {
  ...
  /** PUT: update all students on the server */
  updateAllStudents(students: Student[]): Observable<Student[]> {
    return this.http.put<Student[]>(this.studentsUrl, students, httpOptions).pipe(
      tap((studentsList: Student[]) => this.log(`updated all students, size = ${studentsList.length}`)),
      catchError(this.handleError<Student[]>('updateAllStudents'))
    );
  }

  /** PATCH: update part of the student on the server */
  patchStudent(updates: any, id: number): Observable<any> {
    return this.http.patch(`${this.studentsUrl}/${id}`, updates, httpOptions).pipe(
      tap(_ => this.log(`patched student id=${id}`)),
  ...
  getMyProfile(): Observable<Student> {
    ...
    return this.http.get<Student>(url).pipe(
      tap(_ => this.log(`fetched my profile`)),
      catchError(this.handleError<Student>(`getMyProfile`))
    );
  }
